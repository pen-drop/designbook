...
prompt +=
  ...
  "Plan: follow the shared builder modes (`ephemeral` | `persist` | `ask`; caller/case override wins). Run `_debo intake <workflow> --palette`, author the complete `tasks.json` (every step; each task's params satisfy its params_schema; resolve every open selector), then `_debo plan build <workflow> --tasks <tasks.json>` — add `--ephemeral` when the case or intake default is ephemeral. Durable persist writes `$DESIGNBOOK_DATA/plans/<workflow>.plan.md`; ephemeral writes `$DESIGNBOOK_DATA/plans/.ephemeral/<unique>.plan.md` and must leave the durable canonical path absent. Fix any reported unmet param or missing step and re-run until it returns ok.\n" +
