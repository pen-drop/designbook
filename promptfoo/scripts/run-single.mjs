...
prompt +=
  ...
  "Plan: follow the shared builder modes (`ephemeral` | `persist` | `ask`; caller/case override wins). Run `_debo intake <workflow> --palette`, author the complete `tasks.json` (every step; each task's params satisfy its params_schema; resolve every open selector), then `_debo plan build <workflow> --tasks <tasks.json>` — add `--ephemeral` when the case or intake default is ephemeral, or `--name <a concrete initiative name>` for a durable persist (required unless `--ephemeral`/`--output` is given). Durable persist writes `$DESIGNBOOK_DATA/plans/<date>-<slug>/plan.md`; ephemeral writes `$DESIGNBOOK_DATA/plans/.ephemeral/<unique>.md` and must leave the durable folder absent. Use the CLI's returned `plan` path for every later command — never reconstruct one from `plans_dir`. Fix any reported unmet param or missing step and re-run until it returns ok.\n" +
  "Execute: for `persist`, stop after plan build without execute-workflow. For `ephemeral` or when the case requires execution, run the execute-workflow skill on that plan — loop `_debo plan steps <plan>`, `_debo plan instructions <plan> --step <id>`, produce each task's declared outputs, `_debo plan done <plan> --task <name> --data-file <result.json>` (add `--title` when a step repeats a task name) — until `_debo plan summary <plan>` reports every task done. Leave the sealed plan on disk for scoring (do not delete an ephemeral plan before this run ends). Report every saved path, failure, retry and unanswered input. " +
