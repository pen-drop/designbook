# Drupal Form Blueprint Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a single blueprint that defines the complete Drupal form component system (form_element, label, input, select, textarea, checkbox, radio, checkboxes, radios, submit).

**Architecture:** One blueprint markdown file in the designbook-drupal skill. Follows the existing blueprint format (YAML frontmatter + markdown sections). The form_element wrapper is the central pattern; input types are listed as a flat catalog.

**Tech Stack:** Markdown with YAML frontmatter, designbook skill system

**Spec:** `docs/superpowers/specs/2026-04-15-drupal-form-blueprint-design.md`
...
- **Create:** `.agents/skills/designbook-drupal/components/blueprints/form.md` — the form blueprint

---

### Task 1: Create the form blueprint

**Files:**
- Create: `.agents/skills/designbook-drupal/components/blueprints/form.md`

- [ ] **Step 1: Load the designbook-skill-creator skill**

Per CLAUDE.md, this is required before modifying any file under `.agents/skills/`.

- [ ] **Step 2: Create the blueprint file**

Create `.agents/skills/designbook-drupal/components/blueprints/form.md` with the following content:
...
- [ ] **Step 3: Verify the blueprint is well-formed**

Run from repo root:

```bash
# Check frontmatter parses correctly
head -15 .agents/skills/designbook-drupal/components/blueprints/form.md

# Verify embeds list matches component headings in the file
grep "^### " .agents/skills/designbook-drupal/components/blueprints/form.md
```

Expected: frontmatter shows all 10 embeds, grep shows 10 component headings (form_element, label, input, select, textarea, checkbox, radio, checkboxes, radios, submit).

- [ ] **Step 4: Commit**

```bash
git add .agents/skills/designbook-drupal/components/blueprints/form.md
git commit -m "feat(designbook-drupal): add form blueprint for Drupal form component system"
```
