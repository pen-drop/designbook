...
import { readSummary, evaluateMetric } from '../workflow-summary.js';
...
describe('evaluateMetric', () => {
  function setupWithChild() {
    const root = resolve(tmpdir(), `metric-${Date.now()}`);
    const dataDir = resolve(root, 'designbook');
    mkdirSync(dataDir, { recursive: true });
    writeArchiveWithChildren(dataDir, 'design-shell-xyz', { flow_rate: 80 }, [
      { name: 'design-verify-abc1', workflow: 'design-verify' },
  ...
  it('returns null for a non-numeric result', async () => {
    const { root, r } = setupWithChild();
    const metric = await evaluateMetric(r, 'after.`design-verify`.`score-report`');
    expect(metric).toBeNull();
    rmSync(root, { recursive: true });
  });
  ...
  it('returns null for invalid syntax', async () => {
    const { root, r } = setupWithChild();
    const metric = await evaluateMetric(r, '][');
    expect(metric).toBeNull();
    rmSync(root, { recursive: true });
  });
});
