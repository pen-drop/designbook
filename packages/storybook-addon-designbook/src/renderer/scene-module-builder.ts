...
function resolveLayoutReference(
  ...
  console.log(
    `[Designbook] Layout reference "${ref}" resolved → scene "${target!.name}" with slots: [${Object.keys(target!.layout).join(', ')}]`,
  );
...
export async function buildSceneModule(
  ...
  for (const scene of parsedScenes) {
    ...
    const mergedLayout = layoutSlots ? { ...layoutSlots, ...scene.layout } : scene.layout;
