...
/**
 * Daemon-backed scene inventory check, injected by the composition root
 * (`cli.ts`) via `registerSceneInventoryChecker`. Keeping it an injected seam —
 * rather than importing it here — is what keeps `validation` daemon-free: a pure
 * `validate` run never wires a checker, so it never reaches the Storybook daemon
 * (DESIGNBOOK-60 AC-2). `workflow done` wires the real checker from `tools`.
 */
...
const validators: Record<string, ValidatorFn> = {
  ...
  scene: async (file, config) => {
    ...
    // Safety-net: re-check component ids against live inventory. The inventory
    // check is a daemon-backed seam injected by the composition root; when it is
    // not wired (a pure daemon-free `validate` run) the build result stands.
    if (!sceneInventoryChecker) return buildResult;

    // YAML re-parse failures fall through to buildResult (validateSceneBuild
    // would have caught them), but inventory resolver crashes MUST surface as a
    // scene failure — otherwise the safety-net silently defeats itself.
