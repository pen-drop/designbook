...
**Reason**: Replaced by structured JSON data validated by `schema/data-model.json` for better programmatic usage.
**Migration**: The `designbook-data-model` skill handles the new JSON format. Existing markdown files should be migrated to JSON manually or via a script.
