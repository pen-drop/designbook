...
**Reason**: Replaced by the updated requirement below. The old implementation spawned `npx vitest run --project=storybook` and required `drupal.theme` to be set. New implementation uses Vite SSR and reads `storybook.configDir`.
