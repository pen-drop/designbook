...
### Requirement: Setup-compare step creates story entity and returns checks

A step `design-shell:setup-compare` (and `design-screen:setup-compare`) SHALL create the story entity via `_debo story --scene <scene> --seed <meta-json>` and return the `checks` array as params for the capture and compare stages.
