...
All validate subcommands SHALL use a consistent output format:
- Errors prefixed with `Error: `
- Warnings prefixed with `Warning: `
- Success messages: `<artifact-type> valid`
- Summary line: `N errors, N warnings`

#### Scenario: output format with errors and warnings
- **WHEN** validation produces both errors and warnings
- **THEN** errors SHALL be printed first, then warnings, then the summary line
