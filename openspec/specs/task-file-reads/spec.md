# task-file-reads Specification

## Purpose
Defines the `reads:` frontmatter convention for task files — declaring input file dependencies that must exist before a task executes, and the AI behavior when dependencies are missing.
