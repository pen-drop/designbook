...
**Use for:** Any form rendering — contact forms, login, registration, settings, search, filters.
...
#### Props
- type: string — form element type (text, email, checkbox, select, etc.)
...
Lean input for text-based entries. Type is controlled via the HTML `type` attribute (`v-bind`).

**Use for:** text, email, password, number, tel, url, search, date, file, color, range
...
#### Slots
- option_elements — custom option elements (overrides options prop)
- children — fallback for raw markup
...
#### Slots
- children — raw content (overrides value prop)
...
Named `<slot>` elements per structural region — see the `VueComponent` schema in `components/schemas.yml` for the full constraint. Two-way state (v-model) is not required here: a form component receives its value and emits through props/events the same way any other Vue component in this project does.

## Covered Form Element Types

| Form element type | Blueprint component | Notes |
|---|---|---|
| text, email, password, number, tel, url, search | input | type via HTML attribute |
