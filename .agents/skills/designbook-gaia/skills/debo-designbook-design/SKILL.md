...
description: Own the diagnose, spec, coding, and review steps for a work:design-to-designbook sub-work — plan with debo --plan, build with debo --from-plan, validate with debo design-verify (recording its statistics as the design_verify measurement and resolving the final Storybook link), each step with its own transition.
when:
...
work_type_term:
  ...
  description: "Sub-work: design change realized in Designbook — planned with debo --plan, built with debo --from-plan, validated with debo design-verify."
inputs:
  spec:
    description: how to produce the design plan in the spec step (debo, run to the last interactive stage, writing a plan file)
    default: debo <design workflow> --plan   # e.g. debo design-screen --plan / debo design-entity --plan
  build:
    description: how to build the design autonomously from the spec plan
    default: debo <design workflow> --from-plan <plan>   # e.g. debo design-screen --from-plan <plan>
  validate:
    description: command that validates the Designbook artifacts
    default: debo design-verify
  provision:
...
sub-work. The Designbook flow is **plan → build → validate**: `spec` runs the debo workflow with
`--plan` to write the plan and stop; `coding` runs it with `--from-plan` to build autonomously from
that plan; `validate` (`debo design-verify`) checks the result. One run works one step; do only the
step matching the ticket's current state, then STOP. Every value you reference (`spec`, `build`,
`validate`, `provision`) has a short description and a default in this skill's `inputs`; a project
overrides any inline under this skill's bullet in `WORKFLOW.md` (effective = override ?? default).
...
1. Shared start.
2. Produce the design plan by running `spec` (`debo <design workflow> --plan`): it runs the debo
   workflow up to its last interactive stage, writes the plan file, and stops — no artifacts are
...
3. Publish the gaia `spec` + `test` handoff (design decision, alternatives, risks, `Task-Art`, the
   ...
   `debo design-verify` evidence). Commit the plan.
...
3. Author the QA artifacts: `@gaia/acceptance` → `@gaia/scenario` → the concrete check →
   `@gaia/verify` with `validate` (`debo design-verify`). **RED gate:** the reported design defect
...
2. Invoke `@gaia/implement-ticket` to **build** the design artifacts autonomously from the spec
   plan by running `build` (`debo <design workflow> --from-plan <plan>`) — it reads the decisions
   from the plan written in `spec` instead of asking, and runs the deterministic stages to
   completion. A `gaia_bug` has no `spec` plan; build directly with the debo workflow, following the
   Designbook (`debo`) skill. **The debo build is no exception**: it goes through
   `@gaia/implement-ticket`, which owns the build-dispatch mechanism and runs it in an isolated
   subagent with a self-contained handoff and no credentials/no comment-publishing, returning its
   artifacts/evidence; this parent keeps confirmation, transition, and notification.
3. Drive the acceptance criteria to GREEN. For a feature or chore author the QA artifacts now
   ...
   `diagnose` artifacts. Then invoke `@gaia/verify` with `validate` (`debo design-verify`) and fix
   until every applicable criterion reports GREEN. **GREEN gate:** do not proceed until the
   `debo design-verify` verdict is green; include the component paths and the verdict in the handoff.
4. **Record the measurement.** From the `debo design-verify` `ScoreReport`, record the `design_verify`
   measurement (`measurements/definitions/design-verify.json`: `score`, `delta`,
...
5. Invoke `@gaia/run-outtake`, leading with the `debo design-verify` verdict and its statistics, and
   the **final Storybook link** for the built component(s).
...
2. Invoke `@gaia/review-ticket` in a review subagent: run `@gaia/verify` with `validate`
   (`debo design-verify`) **fresh**, re-validating each acceptance criterion from its abstract
   ...
   `design_verify` measurement from this fresh `debo design-verify` `ScoreReport` (session PATCH,
   before the transition).
3. Invoke `@gaia/run-outtake`, leading with the verdict, the `debo design-verify` statistics, and
   the final Storybook link; for `Not OK` the failing criterion.
