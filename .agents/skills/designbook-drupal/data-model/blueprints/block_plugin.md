...
`user_login_block`) and it also accepts a content block referenced by its plugin form
(`block_content:<uuid>`). Its `plugin` ID is the discriminator on two axes: which
`settings` shape applies, and which SDC renders it.
...
The per-entry shape — the `plugin` ID form, the render `component`, the render `layout`,
the providing `module`, and the family-discriminated `settings` — is the hard contract
carried by the `BlockPlugin` type in this directory's `schemas.yml`, injected into
`config.block_plugin` by this blueprint's `extends:`. Author entries against that type;
this body describes only the surrounding intent, export behaviour, dependencies, and
non-goals.
...
- **Providing module.** The four core families (views, user, block_content) need no module
  entry. A block from a non-core provider must name its providing `module`; a non-core
  plugin left without one is a `config-verify` failure, not a silently empty block.
