...
For the JSONata at the generator path: read the matching blueprint's `### to_drupal` block (the blueprint for `unit.entity_type` + the `field-types` prelude for field units, or the config-type blueprint for config-slice units). For an entity-view-display unit (`config_name` of the form `core.entity_view_display.*`), instead read the display-mapping blueprint matching the view mode's `template` and the active `extensions` — not the entity-type blueprint. This is the pattern to follow when authoring the transform. Run the authored JSONata over `unit` (binding `unit.entity_type`, `unit.bundle`, `unit.field_name`, `unit.def` as needed) to produce the config payload written to `{{ file }}`.
