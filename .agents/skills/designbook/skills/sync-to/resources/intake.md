---
name: sync-to-intake
description: Domain decisions required before planning sync-to artifacts.
---

# sync-to intake

Read the model and effective backend configuration. Resolve the filter and enumerate every selected config entity, bundle, display, form mode, field and dependency. Select applicable config-name routing blueprints now. Freeze target config names and output paths before planning transforms.
