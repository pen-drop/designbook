...
When a display unit's `def` carries `template: presenter` (a theme-methods-only surface — a form, pager, or exposed filter), follow the presenter-template blueprint **in addition to** the display-unit pattern: `transform` writes the **presenter-template** (a Twig theme file) to the path and shape that blueprint directs, alongside the unit's config YAML. The config remains the binding; the Twig carries the theme markup UI-Patterns display config cannot express. A UI-Patterns-bindable surface (`template: field-map`) writes config only.
