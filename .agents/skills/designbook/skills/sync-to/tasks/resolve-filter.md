...
**Build form (declarative, not guessed).** The page the Scene renders binds to a page bundle in the data model. Each Scene-derived unit's `build_form` is taken — not guessed — from the `template` of that bundle's **full** view mode. Two build forms ship as the **built-in registrations**: `layout-builder` ⇒ Layout Builder; `canvas` ⇒ Display Builder. `build_form` is **skill-extensible**: a project skill registers a third form by widening `ConfigNameUnit.build_form` from its own rule/blueprint frontmatter (`extends:` enum-union — see the sync-to `SKILL.md`) and shipping the blueprint that expands it, matched by its `trigger.config_name` glob. Dispatch resolves a skill-registered form exactly as it resolves the two shipped ones, so the concrete config naming and shape (delegated below) is selected without guessing.
...
The concrete config names, the per-build-form shape, and how the Scene's component tree is embedded in the config are delegated to the build form's Drupal blueprint — the two shipped forms are `layout-builder` / `canvas`; a skill-registered form ships its own — WHAT here, HOW there.
