...
result:
  section-scenes:
    path: $DESIGNBOOK_DATA/sections/{{ id }}/{{ id }}.section.scenes.yml
    type: object
    validators: [scene]
each:
  section:
...
```yaml
id: {{ id }}
group: "Designbook/Sections/{{ title }}"
title: "{{ title }}"
...
- **`group:`** must always be `"Designbook/Sections/{{ title }}"` — required in both workflows
