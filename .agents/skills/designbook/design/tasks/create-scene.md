...
The allowed node types (`component:`, `entity:`, `scene:`, `image:`) and their formats are defined by the loaded constraint rules — see `scenes-constraints.md` (core) and the active integration's `scenes-constraints.md` (e.g. Drupal) for exact format specifications.
