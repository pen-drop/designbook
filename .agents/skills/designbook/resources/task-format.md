...
params:                                       # task-specific: declared parameters
  $ref: ../schemas.yml#/Check                   # resolve schema properties as params
  scene: { type: string }                       # explicit entries override/extend $ref
...
`params:` supports a top-level `$ref` that resolves a schema and uses its `properties` as param declarations. Explicit sibling entries override or extend the resolved properties:

```yaml
params:
  $ref: ../schemas.yml#/Section       # resolves Section.properties as params
  order: { type: integer, default: 1 } # override: add default to schema property
  scene: { type: string }              # extend: add param not in schema
```

This avoids duplicating field declarations between `params:` and `each:` when both reference the same schema. The `$ref` must point to an object schema with `properties`.
