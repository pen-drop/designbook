...
files:
  - $DESIGNBOOK_OUTPUTS_CONFIG/sections/{{ section_id }}/data.yml
---

# Create Sample Data

Writes `$DESIGNBOOK_OUTPUTS_CONFIG/sections/{{ section_id }}/data.yml` with realistic sample records. Idempotent: checks existing records and only appends what is missing. Never overwrites existing data. Never writes a `_meta` key.

## Step 1: Read existing data.yml

Read `$DESIGNBOOK_OUTPUTS_CONFIG/sections/{{ section_id }}/data.yml` if it exists. Build an inventory:
...
Check against `$DESIGNBOOK_OUTPUTS_CONFIG/data-model.yml` before writing:
