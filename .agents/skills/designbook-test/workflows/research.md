...
- `metric` — JSONata expression evaluated against the `eval-score.mjs` JSON output (which extends the `workflow summary --json` shape); selects the value used as the decision metric throughout the research loop. Backtick-quote path segments containing dashes (e.g. `` `design-verify` ``). Defaults to `flowRate` if omitted. **KNOWN BROKEN (see issue #129):** the `after.`design-verify`.`score-report`…` metric above no longer resolves — design-verify used to run as an after-hook child of the main run and nest its score-report under `after.design-verify`, but that hook was removed. The research driver does not yet run a validate workflow, so this metric evaluates to `null` and crashes the case. Do not use a design-verify-scored metric until #129 wires validate into the research driver.
