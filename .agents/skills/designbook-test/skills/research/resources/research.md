...
Loaded by `skills/research/SKILL.md` when `debo-test research <suite> <case>` is invoked.
...
| `$WORKSPACE` | `--workspace <path>`, resolved from the repository root; default = `workspaces/$SUITE` |
...
```yaml
metric: scoreReport.successRate
direction: min
```
...
**Parallel runs.** Pass a different `--workspace <path>` to every concurrent
loop. The runner rebuilds that directory and gives it a separate theme git
repository and ddev project. A single sequential run may omit the option and
uses `workspaces/$SUITE`.

For parallel loops that also need isolated source worktrees, provision each run
with:
...
2. Run `./scripts/setup-workspace.sh $SUITE --into "$WORKSPACE"`. This deletes any prior workspace and rebuilds from scratch (rsync, symlinks, `git init`, `pnpm install`, baseline commit).
3. Start Storybook via the addon CLI from the designbook working dir (`$WORKSPACE/web/themes/custom/test_integration_drupal`):
   ```
   ...
   ```
   If the train or any validation case uses Drupal (`ddev`, `sync-to`, or
   `sync-verify`), also run `./scripts/start-drupal-workspace.sh --workspace "$WORKSPACE"`.
4. **Tag the case-agnostic workspace baseline BEFORE layering any case fixtures:**
   `cd "$WORKSPACE/web/themes/custom/test_integration_drupal" && git tag workspace-baseline` (the theme dir is the workspace git repo root). The baseline must contain
...
1. Reset + clean the workspace to the case-agnostic baseline (run in the workspace theme dir — the git repo root):
   ...
   ```
1b. **(sync cases only) Reset Drupal config state to baseline:** if `cases/c.yaml` contains `expected_config:`, run `./scripts/reset-drupal-config.sh --workspace "$WORKSPACE"`. The `git reset` in step 1 reverts the filesystem but NOT the live Drupal DB; without this re-import, a prior case's synced config stays active and poisons this case's existence-check.
2. Layer case `c`'s fixtures: `./scripts/setup-test.sh $SUITE c --into "$WORKSPACE"`.
3. Run case `c`'s prompt via a **driver subagent** (do NOT run the workflow inline on
   ...
   **research runs never have a user.**
   ...
   - Instruct it to perform the domain intake, author and validate one complete
     definition, persist it with `workflow create`, then execute that saved path
     with `execute-workflow`, using the per-task isolation contract from run.md.
     The driver must keep the definition copy made before execution and compare it
     with the saved document after completion.
   - **No user, ever:** if a task needs a decision it cannot answer from the case
...
2. Run audit one last time (over the train case) per [`audit criteria`](../../../resources/audit-criteria.md) → `research-runs/<slug>/final-audit.md`.
